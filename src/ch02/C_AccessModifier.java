...
class AccessModifier {
	...
	void method() {
		...
		// C_AccesModifier 클래스의 privateVariable 필드는 접근 제어자가 private 이기 때문에 외부 클래스에서는 접근이 불가능
		// instance.privateVariable = 0;
	}
}

//ch02_sub의 C_AccessModifierSub 클래스와 같이 참조
//public은 하나의 파일에 하나만 가능
public class C_AccessModifier {
	private int privateVariable;
	int defaultVariable; // 접근 제어자 default
	protected int protectedVariavble;
	public int publicVariable;
	
	void method() {
		// C_AccesModifier 클래스의 privateVariable 필드는 접근 제어자가 private 이기 때문에 클래스 내부에서만 사용 가능
		this.defaultVariable = 0;
	}
}
