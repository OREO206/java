...
// JVM의 메모리 구조
// 메서드 영역: 모든 쓰레드가 공유하며 클래스의 구조 정보와 static으로 선언 된 변수 혹은 메서드가 저장되는 공간
// 힙 영역: 모든 쓰레드가 공유하며 클래스로 생성된 인스턴스와 배열이 저장되는 공간(가비지 컬렉션의 주요 대상)
// 스택 영역: 각 쓰레드별로 존재하는 공간, 로컬 변수나 메서드 호출 데이터가 저장되는 공간(각 메서드가 종료되면 제거됨)
...
public class A_JVM {
	...
	public static void main(String[] args) {
		...
		// 호출한 메서드가 종료되면 스택 영역에서 자동으로 제거 된다.
